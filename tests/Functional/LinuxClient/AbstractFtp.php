...
use JuanchoSL\FtpClient\Adapters\LinuxClientAdapter;
...
abstract class AbstractFtp extends TestCase
{
    ...
    public function setUp(): void
    {
        ...
        if (empty($this->adapter)) {
            ...
            $this->adapter = new LinuxClientAdapter($this->ftp);
        }
    }
    ...
    {
        $this->assertTrue($this->adapter->cd($this->my_dir), "irectory has been changed");
        $this->assertStringEndsWith($this->my_dir, $this->adapter->pwd(), "Current directory is the selected directory");
    }

    public function testUploadFile()
    {
        $this->assertTrue(empty($this->adapter->ls($this->my_dir)), "Directory is empty");
        ...
        $this->assertContains($this->my_file_name, $this->adapter->lsFiles(), "The file is into directory");
    ...
    public function testRenameFile()
    {
        ...
        $this->assertTrue($this->adapter->mv($this->my_file_name, $new_file_name), "Rename the file");
    }
    ...
    {
        $this->assertTrue($this->adapter->cd($this->my_dir), "change to the desired dir");
        $this->assertTrue($this->adapter->rm($this->my_file_name . ".old"), "Delete the file");
        $this->assertTrue(unlink($this->my_file_path . DIRECTORY_SEPARATOR . $this->my_file_name . ".old"), "Delete local file");
        $this->assertTrue(empty($this->adapter->ls()), "The dir is empty");
    }

    public function testDeleteDir()
    {
        $this->assertTrue($this->adapter->rm($this->my_dir), "Delete directory");
    }
