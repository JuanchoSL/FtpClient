...
class SFtp extends AbstractClient implements ConnectionInterface, FilesInterface, DirectoryInterface
{
    ...
    public function stat(string $request): array
    {
        ...
        foreach ($paths as $path) {
            ...
            $full_path = (string) (new StringsManipulators($basedir))->concatenation($path, '/')->replace('//', '/');
            ...
            $result = [
                ...
                $size => $res['size'],
                'unique' => '',
                'UNIX.mode' => (string) (new StringsManipulators(decoct($res['mode'])))->substring(-4),//substr(decoct($res['mode']), -4),
